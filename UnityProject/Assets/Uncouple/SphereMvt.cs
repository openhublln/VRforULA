...
public class SphereMvt : MonoBehaviour
{
    ...
    public float TranslateSpeed = 0.0000000001f;            // Sphere translation speed
    ...
    void Update()
    {
        ...
        CorrectTranslation(-6, 6);                                       // Limit translation between -6 and 6
    }

    void CorrectTranslation(float minPosition, float maxPosition)
    {
        if (transform.position.x < minPosition) { transform.position = new Vector3(minPosition, 9, 10); }
        if (transform.position.x > maxPosition) { transform.position = new Vector3(maxPosition, 9, 10); }
    }
}
